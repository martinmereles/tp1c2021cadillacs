...
int inicializar_esquema_memoria(t_config* config_general, t_config* config_test){
    ...
    if(strcmp(string_algoritmo_ubicacion,"SEGMENTACION")==0){
        ...
        // Inicializo lista de segmentos global
        lista_segmentos = list_create();
        // Agrego un segmento libre del tamanio de la memoria principal
...
void liberar_estructuras_memoria(t_config* config){
    ...
    if(strcmp(config_get_string_value(config, "ESQUEMA_MEMORIA"),"SEGMENTACION")==0){
        ...
        list_destroy_and_destroy_elements(lista_segmentos, destruir_segmento);
    } 
