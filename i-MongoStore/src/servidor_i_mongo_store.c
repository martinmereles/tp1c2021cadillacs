    ...
    if(nombre_parametros[1]==NULL){
        ...
        switch(codigo_tarea){
            ...
            case GENERAR_COMIDA:{
                ...
                if(!existe_archivo(path)){
                    //creando archivo Comida.ims de 0
                    int resto = atoi(nombre_parametros[1])%super_bloque.blocksize;
                    cantidad_de_bloques=atoi(nombre_parametros[1])/super_bloque.blocksize;
                    if(resto!=0){
                        cantidad_de_bloques++;
                    ...
                    for(int i = 0; bloques_usables[i]!=NULL;i++){
                        ...
                        if(i==cantidad_de_bloques-1){
                            //string_append(&bloques_config,bloques_usables[i]);
                            char * fill = string_repeat('C',resto);
                            memcpy(blocksmap+offset,fill,resto);
                        }else{
                            //string_append(&bloques_config,bloques_usables[i]);
                            //string_append(&bloques_config,",");
                            
                            char * fill = string_repeat('C',(int)super_bloque.blocksize);
                            memcpy(blocksmap+offset,fill,super_bloque.blocksize);
                        }
                    }
                    //string_append(&bloques_config,"]");
                    msync(blocksmap, blocks_stat.st_size, MS_SYNC);
                    config_set_value(tarea_config_file,"CARACTER_LLENADO","C");
                    set_tarea_config(
                        tarea_config_file,
                        nombre_parametros[1],
                        string_itoa(cantidad_de_bloques),
            ...
            case CONSUMIR_COMIDA:{
                ...
                if(existe_archivo(path)){
                    ...
                    if(tarea_config.size > size_a_modificar){
                        ...
                        config_save(tarea_config_file);
                        log_info(logger, "se intento consumir mas unidades de comida que las disponibles");
                    }
                    
                }else{
                    log_info(logger,"No se encontro archivo Comida.ims");
                }
                break;
            }
            case GENERAR_BASURA:{
                ...
                if(!existe_archivo(path)){
                    //creando archivo Basura.ims de 0
                    int resto = atoi(nombre_parametros[1])%super_bloque.blocksize;
                    cantidad_de_bloques=atoi(nombre_parametros[1])/super_bloque.blocksize;
                    if(resto!=0){
                        cantidad_de_bloques++;
                    ...
                    for(int i = 0; bloques_usables[i]!=NULL;i++){
                        ...
                        if(i==cantidad_de_bloques-1){
                            //string_append(&bloques_config,bloques_usables[i]);
                            char * fill = string_repeat('B',resto);
                            memcpy(blocksmap+offset,fill,resto);
                        }else{
                            //string_append(&bloques_config,bloques_usables[i]);
                            //string_append(&bloques_config,",");
                            
                            char * fill = string_repeat('B',(int)super_bloque.blocksize);
                            memcpy(blocksmap+offset,fill,super_bloque.blocksize);
                        }
                    }
                    //string_append(&bloques_config,"]");
                    msync(blocksmap, blocks_stat.st_size, MS_SYNC);
                    config_set_value(tarea_config_file,"CARACTER_LLENADO","B");
                    set_tarea_config(
                        tarea_config_file,
                        nombre_parametros[1],
                        string_itoa(cantidad_de_bloques),
